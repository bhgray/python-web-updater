...
letterday = ''
dodapage = 1
defaulturl = 'http://www.teachopensource.org/cs/'
...
def main(argv):
	...
	try:
		opts, args = getopt.getopt(argv, "c:m:d:y:l:sh", ["class=","month=", "day=", "year=","letterday=", "syllabus", "help"])
	...
	for opt, arg in opts:
		...
		if opt in ("-c", "--class"):
			...
			letterday = arg
		elif opt in ("-s", "--syllabus"):
			# get syllabus from server and update it...
			dodapage = 0
		elif opt in ("-h", "--help"):
			usage()
			sys.exit(2)
